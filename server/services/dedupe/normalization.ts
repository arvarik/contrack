 ...
 * `task_type` enum).
 */
export function contactToEmbeddingString(contact: NormalizedContact, raw: RawContactRow): string {
  const parts: string[] = [];
...
export function normalizeContact(
  ...
  const lastNameNorm = nameTokens.length > 1 ? nameTokens[nameTokens.length - 1] : nameTokens[0] ?? "";

  // Phonetic hashes
  const phoneticHash = doubleMetaphone(raw.name).primary;
  const firstNamePhonetic = firstNameNorm ? doubleMetaphone(firstNameNorm).primary : "";
  const lastNamePhonetic = lastNameNorm ? doubleMetaphone(lastNameNorm).primary : "";
...
export function normalizeContacts(
  ...
  const allEmails = sqlite.prepare(
    "SELECT contactId, email FROM contact_emails"
  ...
  const allPhones = sqlite.prepare(
    "SELECT contactId, phone FROM contact_phones"
  ...
  const allSources = sqlite.prepare(
    "SELECT contactId, platform FROM contact_sources"
  ).all() as { contactId: string; platform: string }[];

  const sourcesByContact = new Map<string, string[]>();
  for (const s of allSources) {
    if (!sourcesByContact.has(s.contactId)) sourcesByContact.set(s.contactId, []);
  ...
  for (const raw of allContacts) {
    ...
    normalized.push(normalizeContact(
      raw,
