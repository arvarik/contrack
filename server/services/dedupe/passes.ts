...
import { normalizePhone, normalizeCompany, isNicknameMatch } from "../../utils/nlp/index.ts";
import { buildBlockIndex, generateCandidatePairs, addEmbeddingCandidates, isKnownDistinct, pairKey } from "./blocking.ts";
...
export function buildScoringReasoning(signals: MatchSignals, score: number, rawA: any, rawB: any): string {
  const parts: string[] = [];

  if (signals.nameExactMatch) parts.push(`exact name match`);
  else if (signals.nicknameMatch) parts.push(`nickname match ("${rawA?.name}" ↔ "${rawB?.name}")`);
  else if (signals.nameJaroWinkler >= 0.85) parts.push(`high name similarity (${(signals.nameJaroWinkler * 100).toFixed(0)}%)`);
  ...
  if (signals.embeddingSimilarity > 0.5) parts.push(`embedding similarity ${(signals.embeddingSimilarity * 100).toFixed(0)}%`);

  const reasoning = parts.length > 0 ? parts.join(", ") : `composite score ${(score * 100).toFixed(0)}%`;
...
export function runDeterministicPass(ctx: PassContext): RawPair[] {
  ...
  for (const m of emailDupes) {
    ...
    pairs.push({
      idA: m.id1, idB: m.id2,
      matchType: 'email', confidence: 0.98,
  ...
  for (const [normPhone, contactIds] of phoneMap) {
    ...
    for (let i = 0; i < unique.length; i++) {
      for (let j = i + 1; j < unique.length; j++) {
        ...
        pairs.push({
          idA: unique[i], idB: unique[j],
          matchType: 'phone', confidence: 0.95,
  ...
  // D3: Exact name match
  const nameDupes = sqlite.prepare(`
  ...
  const allSources = sqlite.prepare("SELECT contactId, platform FROM contact_sources").all() as { contactId: string; platform: string }[];
  const sourcesByContact = new Map<string, Set<string>>();
  for (const s of allSources) {
    if (!sourcesByContact.has(s.contactId)) sourcesByContact.set(s.contactId, new Set());
  ...
  for (const m of nameDupes) {
    ...
    const companyA = nA?.companyNorm ?? normalizeCompany(contactMap.get(m.id1)?.company ?? "");
    const companyB = nB?.companyNorm ?? normalizeCompany(contactMap.get(m.id2)?.company ?? "");
    const sameCompany = companyA.length > 1 && companyB.length > 1 && companyA === companyB;

    const srcA = sourcesByContact.get(m.id1);
    const srcB = sourcesByContact.get(m.id2);
    const isCrossSource = srcA && srcB && srcA.size > 0 && srcB.size > 0 &&
  ...
  for (const n of ctx.normalized) {
    ...
    if (!lastNameGroups.has(n.lastNameNorm)) lastNameGroups.set(n.lastNameNorm, []);
  ...
  const d1d2 = pairs.filter(p => p.matchType === 'email' || p.matchType === 'phone').length;
  const d3d6 = pairs.length - d1d2;
  log.info("DedupeService", `[${rid}] Deterministic pass: ${pairs.length} pairs (D1/D2: ${d1d2}, D3–D6: ${d3d6})`);
  return pairs;
}

export async function runFunnelPass(ctx: PassContext, scanId?: string, useEmbeddings = true): Promise<RawPair[]> {
  ...
  // 1: Blocks
  if (scanId) {
    dedupeQueue.update(scanId, { phase: 'blocking', phaseName: 'Building candidate blocks…' });
  }

  const blockIndex = buildBlockIndex(normalized);
  const { candidates: blockCandidates, stats } = generateCandidatePairs(blockIndex, seenPairs);
  log.info("DedupeService", `[${rid}] Blocking: ${normalized.length} -> ${stats.totalCandidates} candidates`);
  ...
  if (useEmbeddings) {
    ...
    log.info("DedupeService", `[${rid}] Skipping embedding KNN — embeddings unavailable`);
  }
  ...
  // 4+5: Score
  const aiQueue: { pair: { idA: string; idB: string }; signals: MatchSignals; score: number }[] = [];
  let autoCount = 0;
  let discardCount = 0;
  ...
  for (const candidate of allCandidates) {
    ...
    if (classification === "auto") {
      ...
      pairs.push({
        idA: candidate.idA, idB: candidate.idB,
        matchType: 'fuzzy', confidence: score,
  ...
  if (aiQueue.length > 0 && ai.isConfigured) {
    if (scanId) {
      dedupeQueue.update(scanId, { phase: 'ai', phaseName: `Verifying ${aiQueue.length} ambiguous pairs via AI…` });
    }
      ...
      try {
        ...
        for (const result of aiResults) {
          if (result.isDuplicate && result.confidence >= 0.6) {
            ...
            seenPairs.add(pk);
            pairs.push({
              idA: candidate.a.id, idB: candidate.b.id,
              matchType: 'ai', confidence: result.confidence,
        ...
        for (const candidate of batch) {
          if (candidate.score >= 0.80) {
            ...
            pairs.push({
              idA: candidate.a.id, idB: candidate.b.id,
              matchType: 'fuzzy', confidence: candidate.score * 0.85,
    ...
    for (const item of aiQueue) {
      if (item.score >= 0.75) {
        ...
        pairs.push({
          idA: item.pair.idA, idB: item.pair.idB,
          matchType: 'fuzzy', confidence: item.score * 0.7,
