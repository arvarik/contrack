 ...
 */
function hydrateCandidates(candidateIds: string[], limit: number): Map<string, HydratedMatch> {
  const hydratedMap = new Map<string, HydratedMatch>();
  const topIds = candidateIds.slice(0, limit);

  for (const id of topIds) {
    const fullRow = sqlite.prepare("SELECT * FROM contacts WHERE id = ?").get(id);
    if (!fullRow) continue;
    hydratedMap.set(id, { ...contactRepo.hydrate(fullRow), aiReason: null });
  }
 ...
 */
function buildCompressedCandidates(candidateIds: string[], limit: number): CompressedContact[] {
  ...
  for (const id of candidateIds.slice(0, limit)) {
    ...
    const interests = (interestsStmt.all(id) as { interest: string }[]).map(t => t.interest);
...
export const searchService = {
  ...
  async semanticSearchStream(query: string, rid: string, res: Response) {
    ...
    if (retrieval.candidates.length === 0) {
      ...
      log.info("SemanticSearch", `[${rid}] "${query}" → 0 candidates in ${elapsed}ms`);
      res.write(JSON.stringify({ phase: "complete", matches: [], fallback: false, cached: false, latencyMs: elapsed }) + "\n");
      res.end();
      return;
    }
    ...
    const compressedCandidates = buildCompressedCandidates(candidateIds, RERANKER_LIMIT);

    try {
      const aiMatches = await rerankCandidates(query.trim(), compressedCandidates);
  ...
  async semanticSearch(query: string, rid: string) {
    ...
    try {
      ...
      if (aiMatches.length > 0) {
        ...
        const elapsed = Date.now() - startTime;
        log.info("SemanticSearch", `[${rid}] v3 "${query}" → ${enriched.length} AI matches in ${elapsed}ms — caching`);
