...
export const listService = {
  ...
  addMember(listId: string, contactId: string) {
    const list = sqlite.prepare("SELECT id FROM lists WHERE id = ?").get(listId);
    if (!list) throw new Error("List not found");

    const contact = sqlite.prepare("SELECT id FROM contacts WHERE id = ?").get(contactId);
    if (!contact) throw new Error("Contact not found");

    sqlite.prepare("INSERT OR IGNORE INTO list_members (listId, contactId) VALUES (?, ?)").run(listId, contactId);
    return true;
  },

  removeMember(listId: string, contactId: string) {
    sqlite.prepare("DELETE FROM list_members WHERE listId = ? AND contactId = ?").run(listId, contactId);
    return true;
  },
