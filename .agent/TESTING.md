...
| `tests/integration/tenancy.isolation.test.ts`      | The cross-owner matrix, with two real accounts and auth on. Every route in `server/routes/contacts.ts` plus the `/uploads` guard: a foreign id and an unknown id give the same 404 body, bulk endpoints touch and count only the caller's rows, an import never matches another owner's contact, and A's rows are byte-identical after each attempt. The unconverted routes stay `it.todo`. |
