...
| ---------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `contacts`             | Primary entity boundary             | `id`, `name`, `firstName`, `lastName`, `headline`, `role`, `company`, `location`, `lat/lng`, `isGhost`, `isArchived`, `canonicalId`, `phoneticHash`, `relationshipScore`, `searchExpansion`, `aiBriefing`, `aiSummary`, `cadenceDays`, `birthday` |
...
- `server/routes/auth.ts` - `/api/auth` router mounted pre-gate: status, setup (closes itself once an account exists), login, logout, me, change-password, sessions, passkey registration/login, password reset, and magic link endpoints. Credential endpoints are rate limited per IP.
