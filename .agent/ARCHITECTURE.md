...
- **MUST** treat an in-memory queue or map keyed by a client-supplied id as owned data. The AI Search batch queue (`server/services/aiSearch/jobQueue.ts`) holds each batch beside its owner and refuses a read from anybody else, because no `WHERE` clause protects a `Map`. A per-account limit (the research cooldown) is keyed by owner; a limit that protects a shared resource (the single-batch run lock, which guards one provider API key) stays instance-wide.
