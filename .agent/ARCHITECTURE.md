...
- `server/services/authService.ts` — Accounts, sessions, ownership. Sessions are server-side rows keyed by the SHA-256 of the cookie secret. `claimUnownedData` / `reconcileOwnership` assign `ownerId IS NULL` rows to the sole account — the seam multi-tenancy will build on.
