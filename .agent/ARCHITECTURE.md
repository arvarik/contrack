...
- **MUST** classify every route in `ROUTE_MANIFEST`, and every `scoped` route must be `isolated: true` with a test behind it in `tests/integration/tenancy.isolation.test.ts`. The manifest test fails on an unclassified route, a stale row, a scoped route with no isolation test, and an `admin` route that lost its class. `requireAdmin` is mounted nowhere: Phase 2 classifies, Phase 3 gates.
