...
- **A statement ends without a period.** Every settings page's description, every row's, and the one-line statements on cards, buttons, hints, empty states, dialogs and toasts drop their closing period, and a statement of several sentences drops only its last. `tests/unit/copy.periods.test.ts` holds it for the statement props under `src/`, the settings registry and the destination names.
