...
- **Solid menus.** Dropdown menus, context menus and comboboxes paint a new `.menu-panel` surface: the card colour, a hairline ring, a soft shadow and a 120 ms entrance from the anchoring edge. The sort menu on the Network page used to be glass over the contact list, and rows showed through its items. `ActionMenu` also slides in from the window's edge when the trigger is closer to that edge than the menu is wide, so a menu in the last column is never cut off, and it scrolls past about ten items. Menu items have a neutral hover and a 4 px inner radius. The `nice-scrollbar` class, referenced by a dozen scrollers and never defined, now draws a thin bar in the palette's hairline colour.
