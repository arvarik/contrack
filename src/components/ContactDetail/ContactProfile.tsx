...
import {
  ...
  Sparkles, UploadCloud, Trash2, Globe, MessageSquare, ExternalLink, Linkedin, Facebook, Tag, X, Coffee, File, Archive, Heart, Edit2, ArrowUpRight
} from "lucide-react";
import ReactMarkdown from 'react-markdown';
import {
  useContact, useTimeline, useUpdateContact, useAddAttachment, useDeleteContact,
  useDeleteInteraction, useUpdateInteraction, useGenerateBriefing, usePromoteGhost,
  useArchiveContact, useUnarchiveContact
} from "../../api";
...
import { RichInteractionComposer } from "../RichInteractionComposer";
...
export const ContactProfile = ({ contactId: id, onClose, showNetworkButton = false }: ContactProfileProps) => {
  ...
  const handleAddInterest = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && newInterest.trim() && contact) {
      ...
      updateContact.mutate({ id, data: { interests: updated } as any });
      setNewInterest('');
    }
  };
      ...
      <div className="p-6 md:p-8 lg:px-10 lg:pt-8 lg:pb-6 max-w-6xl mx-auto w-full relative lg:shrink-0">
        ...
        <section className="flex flex-col md:flex-row items-start md:items-center gap-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* Actions: Catch Me Up + Open in Network */}
