  ...
  return (
    <div className="mt-4 flex flex-col gap-2 relative">
      ...
      >
        ...
        {expanded ? <ChevronUp className="w-4 h-4 text-on-surface-variant opacity-50" /> : <ChevronDown className="w-4 h-4 text-on-surface-variant opacity-50" />}
      </button>
        ...
        {expanded && (
          ...
          >
             {completedItems.map(item => (
                <div key={item.id} className={cn(CARD_COMPACT, "opacity-60 saturate-50 hover:opacity-100 hover:saturate-100 transition-all p-3 flex sm:items-center sm:flex-row flex-col gap-2")}>
...
export const DashboardView = () => {
  ...
  const hasActionItems = dashboard && (dashboard.overdue.length > 0 || dashboard.dueToday.length > 0 || dashboard.upcoming.length > 0);
  const firstActionItem = dashboard ? (dashboard.overdue[0] || dashboard.dueToday[0] || dashboard.upcoming[0]) : null;
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (key === 'd') {
        ...
        e.preventDefault();
        updateAction.mutate({ id: firstActionItem.id, data: { dueAt: addDays(new Date(), 1).toISOString() } });
      } else if (key === 'l') {
        e.preventDefault();
    ...
    return (
      <div className="w-full h-full flex items-center justify-center p-8">
        <div className={EMPTY_STATE}>
          ...
          <h2 className="text-xl font-bold text-on-surface mb-2">System Disconnected</h2>
      ...
      <div className="max-w-5xl mx-auto p-6 md:p-10 flex flex-col gap-8 pb-32">
        ...
        {activeTab === 'pulse' && (isDashboardLoading || !dashboard) ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
            ...
            {/* AI Insight */}
            <DailyInsightCard insight={insight} isLoading={isInsightLoading} delay={0.4} />
            ...
            <div className="col-span-full xl:col-span-8 flex flex-col gap-8">
              {!hasActionItems ? (
                ...
                >
                  ...
                  <h2 className="text-2xl font-bold text-on-surface">No Followups</h2>
                </motion.div>
              ) : (
                <div className="flex flex-col gap-6">
                   <ActionItemSwimlane title="Overdue" items={dashboard.overdue} theme="urgent" delay={0.4} firstActionItemId={firstActionItem?.id} />
                   <ActionItemSwimlane title="Due Today" items={dashboard.dueToday} theme="today" delay={0.5} firstActionItemId={firstActionItem?.id} />
                   <ActionItemSwimlane title="Upcoming" items={dashboard.upcoming} theme="upcoming" delay={0.6} firstActionItemId={firstActionItem?.id} />
                </div>
              )}
