...
const SkeletonText = ({
  ...
  <div
    className={cn(
      "h-4 bg-surface-container-highest/60 animate-pulse rounded-full",
      width,
      className,
    )}
  />
    ...
    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start w-full">
      ...
      <div className="col-span-full grid grid-cols-1 md:grid-cols-3 gap-6">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="bg-surface-container-lowest rounded-2xl shadow-sm p-5 flex flex-col items-center justify-center h-32 relative overflow-hidden"
          >
        ...
        {[1, 2].map((section) => (
          <div key={section} className="flex flex-col gap-3">
            ...
            {[1, 2].map((card) => (
              <div
                key={card}
                className="w-full rounded-xl bg-surface-container-lowest shadow-sm p-4 flex items-center gap-4 animate-pulse"
              >
