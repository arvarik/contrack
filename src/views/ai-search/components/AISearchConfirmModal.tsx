...
export function AISearchConfirmModal({
  isOpen, onClose, onConfirm, selectedContacts, isStarting,
}: Props) {
  const total = selectedContacts.length;
  const previouslySearched = selectedContacts.filter(c => c.aiHydratedAt).length;
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Start AI Search">
      <div className="space-y-5 pt-2">
        ...
        <p className="text-sm text-on-surface-variant leading-relaxed">
          You're about to research <span className="font-bold text-on-surface">{total}</span> contact{total !== 1 ? 's' : ''} using
          AI-powered internet search.
        </p>

        {/* Info bullets */}
        <div className="space-y-2.5">
          <InfoRow text={`Uses approximately ${total} AI search credit${total !== 1 ? 's' : ''}`} />
          <InfoRow text={total === 1
        ...
        {previouslySearched > 0 && (
          <p className="text-xs text-on-surface-variant bg-surface-container-low rounded-xl p-3 leading-relaxed">
            <span className="font-bold">{previouslySearched}</span> of these contact{previouslySearched !== 1 ? 's have' : ' has'} been
        ...
        <div className="flex gap-3 pt-1">
          ...
          >
            ...
            {isStarting ? 'Starting…' : `Search ${total} Contact${total !== 1 ? 's' : ''}`}
          </button>
        </div>
