...
| `get_contact`       | Retrieve full contact profile including relationship score explanation.    | `id` (UUID)                                                                                                                   |
| `list_contacts`     | List contacts with optional filtering, sorting, and pagination.            | `limit` (optional, 1–100), `offset` (optional), `updatedSince` (optional ISO 8601), `sort` (optional: name, score, updatedAt) |
...
| `update_contact`       | Update contact properties.                          | `id` (UUID), `patch` (object containing name, company, role, email, phone, location, tags, etc.)                                                        |
